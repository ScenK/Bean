...
// Privacy: carries who/where (sender + channel name), never message text.
export type ChatopsActivity =
  | { type: "turn"; phase: "start" | "end"; id: string; who: string; where?: string; error?: string }
  | { type: "run"; phase: "start" | "tail" | "done" | "failed" | "cancelled"; id: string; name: string; line?: string }
...
const PHASES: Record<ChatopsActivity["type"], readonly string[]> = {
  turn: ["start", "end"],
  run: ["start", "tail", "done", "failed", "cancelled"],
  live: ["start", "end"],
};
...
export function parseChatopsActivity(v: unknown): ChatopsActivity | undefined {
  ...
  const id = str(o.id);
  if (!Object.hasOwn(PHASES, type) || !PHASES[type].includes(o.phase as string) || !id) return undefined;
  ...
  if (type === "run") return { type, phase, id, name: str(o.name) ?? "delegate", line: str(o.line) };
