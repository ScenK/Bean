...
- **The unit of selection is a routine's whole local day**, not one run (`RunBucket` out of
  ...
  end rather than one per run. Every level is newest-first, buckets included: the latest run
  reads at the top of the spine with the "you are here" marker above it, and a run's displayed
  ordinal is its chronological place in the day (`runs.length - index`), so RUN 2 sits above
  RUN 1 rather than the top row claiming to be the day's first. Fold state is a single localStorage list of day headers
