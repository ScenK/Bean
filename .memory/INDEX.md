...
- [project-model-selection-and-no-project-runs.md](project-model-selection-and-no-project-runs.md) — the model picker (canonical names + per-CLI aliases + last-used-per-skill) and the merged "no project" scratch-workspace/URL-seed flow; `buildLaunchHandler` stays sync unless projectPath is "".
