...
- [safety-tray-tahoe-placement.md](safety-tray-tahoe-placement.md) — macOS 26.5 (Tahoe) races tray placement and can park the icon off-screen; `main.ts` keeps a destroy-and-recreate watchdog until `getBounds()` shows real menu-bar coords, and `dock.hide()` must stay after tray creation.
