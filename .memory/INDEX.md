...
- [convention-chatops-addressing-and-ambient.md](convention-chatops-addressing-and-ambient.md) — channel addressing tiers (explicit = full tools, casual name-match = text-only via `proposalsAvailable: false`), ambient block is untrusted data with a time anchor, the chronology/persist/cutoff trio in `bot.ts`, and the `/new` reset command.
