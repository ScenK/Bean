...
- **Ambient block chronology + persistence + cutoff go together** (`bot.ts onMessage`): the
  ...
  `ambientCutoff` (newest injected timestamp) floors the next `fetchRecent` so persisted chatter
  is never re-injected. Removing any one of the three reintroduces duplication or incoherent
  follow-ups. Discord's `fetchRecent` also excludes messages that @mention Bean — those are
  already in history.
- **`/new`** (like the `cancel` text command) clears the conversation via
  `ConversationStore.clear()` and fences ambient with `ambientCutoff = now`, so pre-reset
